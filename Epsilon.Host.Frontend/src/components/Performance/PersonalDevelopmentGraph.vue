...
onMounted(() => {
	const rowTypes = store.state.personalDevelopment.rowsSet.types.sort(
...
const series = computed(() => [
	{
		...
		data: DecayingAverageLogic.getAverageSkillOutcomeScores(
			...
			return {
				...
				x: store.state.personalDevelopment.rowsSet.types
					.sort(
						(a: LearningDomainType, b: LearningDomainType) =>
							(a.order || 0) - (b.order || 0)
					)
					.find((s: LearningDomainType) => s.id == d.skill).name,
...
function getValue(valueId: number | null): LearningDomainType | undefined {
	return store.state.domain.valuesSet.types
		.sort(
			(a: LearningDomainType, b: LearningDomainType) =>
				(a.order || 0) - (b.order || 0)
		)
